...
export class ConnectionManager {
  ...
  private async loadConnectionMap(channel: ConnectionChannel): Promise<ConnectionMap> {
    const channelMap = this.channelMap.get(channel)
    if (channelMap) {
      // this.debug(this.loadConnectionMap, `ConnectionMap already loaded (channel: ${channel})`);
      return channelMap
    }

    const connectionPromiseMap = this.loadConnectionPromiseMap.get(channel)
    if (connectionPromiseMap) {
      this.debug(this.loadConnectionMap, `Loading ConnectionMap in progress (channel: ${channel})`)
      return connectionPromiseMap
    }
  ...
  async getConnection(cid: ConnectionID, channel: ConnectionChannel): Promise<Connection | null> {
    ...
    if (connection) this.debug(this.getConnection, `Connection found (${channel}): ${JSON.stringify(connection)}`)
    else this.debug(this.getConnection, `Connection not found (${channel}): ${cid}`)
  ...
  async getConnections(channel: ConnectionChannel): Promise<Connection[]> {
    ...
    if (connections.length > 0) this.debug(this.getConnections, `Connections found (${channel}): ${connections}`)
    else this.debug(this.getConnections, `Connections not found (${channel})`)
  ...
  private static debug(fn: (...args: any[]) => any, message: any): void
